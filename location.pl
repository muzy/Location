...
my $VERSION = '0.1';
my ($login,$pass,$uid,$gid) = getpwuid($UID);
my $configpath = '/home/'. $login .'/.locationconf';
...
if( -e $configpath ){
	...
	$accuracity = $config->{_}->{accuracity};
}else{
    $config->{_}->{version} = $VERSION;
    $config->{_}->{accuracity} = "ok";
	$config->{_}->{scans} = 1;
	$config->write($configpath);
}


GetOptions ('h|help'	=>	sub { exec('perldoc', '-F', $0) },
			'a|add=s{3}'=>	\@add,
			'd|delete=s'=>	\$delete,
			's|show=s'	=>	\$show,
			'scans=i'	=>	\$scans,
...
else{ say "usage: location [--help] [--add <name> <address> <station>] [--delete <name>] [--show <name>] [--scans <number>] [--delay <time in seconds>] [--get <name|address|station|all>] [--accuracity <ok|best>] [--verbose]";}
...
sub get {
    ...
    while (my ($name, $section) = each %{ $config }){
    	if($name ne "_"){
       		foreach my $mymac (@localmacs){
				if(grep $_ eq $mymac, split(/\|/,$section->{mac})){ push(@possiblelocations,$name); }
        	} 		
    	}
    }
	if(@possiblelocations){
		...
		foreach my $loc (@locations){
			...
			if($accuracity eq "ok"){
				if($diffcount <  $confmaccount){
					...
					if(($get eq "address") xor ($get eq "all")){ say $config->{$loc}->{address}; }
					if(($get eq "station") xor ($get eq "all")){ say $config->{$loc}->{station}; }
				}
			}else{
		...
		say "Error: Could not determine current location!"; exit 1;
	}
}
...
sub scan_environment {
	my @output = qx(sudo iwlist scan 2> /dev/null | grep -i 'Address' | cut -d ":" -f2-); 
	trim @output;
	return @output;
}
...
[--get I<name|address|station|all>] [--accuracity I<ok|best>] [--verbose]

=head1 DESCRIPTION

Location can determine your location. Therefore it scans available wlan networks
